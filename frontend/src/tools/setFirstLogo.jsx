...
const setFirstLogo = (pokemon, index) => {
  let pokemonType = null;

  if (pokemon.types[index]) {
  ...
  for (const type of types) {
    if (type.name === pokemonType) {
      return (
        <div
          className={`bg-[#${type.color}] h-7 w-7 rounded-tr-lg flex justify-center items-center`}
        >
          <img
            className="h-5 w-5 rounded-tr-lg"
            src={type.img}
            alt={`type ${type.name}`}
          />
        </div>
